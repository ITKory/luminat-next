              ...
              {isLastStep ? null : (
                  <div
                      ...
                      className={`absolute left-[2.05rem] top-[1.1rem] h-px w-[calc(100%-2.5rem)] ${
                          index < lastCompletedIndex ? "bg-green" : "bg-[rgba(81,88,98,0.18)]"
                      }`}
                  />
              )}
              ...
              <div className="relative z-10 flex flex-col gap-5">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-bwhite">
                  <Image
                    ...
                    width={32}
                    height={32}
                    className={isActive || isDone ? "opacity-100" : "opacity-70"}
                  />
                </div>

                <p
                  className={`max-w-44 text-[12px] leading-[1.35] md:text-[12px] ${
                    isDone || isActive ? "text-green" : "text-[rgba(81,88,98,0.34)]"
                  }`}
                >
                  {step.title}
                </p>
...
function OrderItemRow({
  ...
  return (
    <div className="grid gap-6 py-6 md:grid-cols-[minmax(0,1.7fr)_9rem_13rem_10rem] md:items-center md:gap-4">
      <div className="flex min-w-0 gap-6 md:gap-8">
        ...
        <div className="min-w-0 self-center space-y-2">
          <div className="text-[36px] leading-[0.94] tracking-[-0.045em] text-green md:text-[36px]">
            {name}
          </div>
          {description ? (
            <p className=" text-[12px]  text-[rgba(81,88,98,0.78)] md:text-[12px]">
              {description}
            </p>
          ) : null}
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 md:contents">
        ...
        <div className="space-y-2 md:self-center">
          ...
          <div className={`max-w-48 text-[20px] leading-[1.15] md:text-[20px] ${getOrderStatusClass(status, statusClassName)}`}>
            {status || "—"}
          </div>
        </div>
        ...
        <div className="space-y-2 md:self-center md:text-right">
          ...
          <div className="text-[36px] leading-none tracking-[-0.05em] text-green md:text-[36px]">
            {formatPrice(total)}
          </div>
        </div>
      </div>
        ...
        <section className="pb-20">
          <div className="container-32 mt-120 grid items-start gap-8 xl:grid-cols-[15rem_minmax(0,1fr)]">
            ...
            <div className="border border-dashed border-[rgba(81,88,98,0.2)] px-6 py-10 md:px-8">
              <div className="text-2xl leading-tight text-green">Заказ не найден</div>
              <p className="mt-3 max-w-xl text-sm leading-6 text-gray md:text-base">
                Проверьте номер заказа или вернитесь к списку заказов.
              </p>
              <Link href="/profile/orders" className="mt-6 inline-flex items-center gap-2 text-green hover:underline">
        ...
        <div className="container-32 mt-120 grid items-start gap-10 xl:grid-cols-[15rem_minmax(0,1fr)]">
          ...
          <div className="min-w-0 space-y-14 xl:max-w-[72rem]">
            ...
            <section className="space-y-5">
              <div className="text-[20px] leading-none tracking-[-0.05em] text-green md:text-[20px]">
                Товары
              </div>

              <div className="hidden grid-cols-[minmax(0,1.7fr)_9rem_13rem_10rem] border-b border-[rgba(81,88,98,0.14)] pb-4 text-[1.08rem] leading-none text-[rgba(81,88,98,0.72)] md:grid md:text-[1.22rem]">
                <span>Товар</span>
            ...
            <section className="space-y-0 pt-2">
              <div className="flex items-center justify-between border-b border-dashed border-[rgba(81,88,98,0.18)] py-3 text-[1.1rem] text-[rgba(81,88,98,0.82)] md:text-[20px]">
                <span>Товары ({lineItemsCount})</span>
              ...
              <div className="flex items-end justify-between border-b border-dashed border-[rgba(81,88,98,0.18)] py-5 md:py-6">
                <span className="text-[1.05rem] leading-none tracking-[-0.05em] text-green md:text-[40px]">
                  Итого
                </span>
                <span className="text-[1.05rem] leading-none tracking-[-0.06em] text-green md:text-[40px]">
